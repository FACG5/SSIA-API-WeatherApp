...
test("test for city&Country", function(t) {
  var url =
    "http://api.openweathermap.org/data/2.5/weather?q=gaza,ps&APPID=" +
    myKey.MY_KEY;
...
test("test for wrong entry", function(t) {
  var url =
    "http://api.openweathermap.org/data/2.5/weather?q=ssssss&APPID=" +
    myKey.MY_KEY;
  myMethod.fetch(url, function(result) {
...
test("Test Next Five Day ", function(t) {
  var url =
    "http://api.openweathermap.org/data/2.5/forecast?q=gaza,ps&APPID=" +
    myKey.MY_KEY;
...
test("Test For Get Forecast Description", function(t) {
  ...
  myMethod.fetch(url, function(result) {
    var actual = myMethod.getNextFiveDay(
      result,
      myMethod.getForecastDescription
    );
    var expected = [
      "clear sky",
      "light rain",
